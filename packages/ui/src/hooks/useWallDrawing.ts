...
export type SnapType = 'endpoint' | 'edge' | 'mesh' | 'foundation'
...
export function isPointOnOrInsideFoundations(point: Point2, foundations: { boundary: [number, number][] }[]): boolean {
...
export function wallSegmentRespectsFoundations(start: Point2, end: Point2, foundations: { boundary: [number, number][] }[]): boolean {
...
export function getFoundationSnap(point: Point2, foundations: { boundary: [number, number][] }[]): SnapResult | null {
...
export interface WallDrawingState {
  isActive: boolean
  pendingWallStart: Point2 | null
  previewEnd: Point2 | null
  cursorPoint: Point2 | null
  snapMarker: SnapResult | null
...
/**
 * Shared hook for wall drawing in both 2D and 3D viewports.
 * Contains ALL wall logic — chain drawing, ortho constraint, foundation
 * containment, wall-to-wall snap, wall creation.
 */
export function useWallDrawing(mode: ViewportMode): WallDrawingState {
  ...
  const handleClick = useCallback((e: ThreeEvent<PointerEvent>) => {
    ...
    if (!pendingWallStart) {
      ...
      setPreviewEnd(point)
      setToolReadout(`Wall start X:${formatLength(x, lengthUnit)} ${mode === '3d' ? 'Z' : 'Y'}:${formatLength(z, lengthUnit)}`)
      return
    }
